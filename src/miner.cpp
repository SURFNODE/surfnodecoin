...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SURFNODEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SURFNODEMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SURFNODEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("surfnode-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SURFNODEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
