...
#if defined(NDEBUG)
#error "SURFNODE cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
    	...
    	return 0.001 * COIN;
    } else if (nHeight >= 251 && nHeight <= 350) {
    	return 1 * COIN;
    } else if (nHeight >= 351 && nHeight <= 500) {
    	return 0.5 * COIN;
    } else if (nHeight >= 501 && nHeight <= 750) { 
    	return 0.25 * COIN;
    } else if (nHeight >= 751 && nHeight <= 1000) { 
    	return 0.15 * COIN;
    } else { 
    	return 0.1 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 19) {
      ret = blockValue * 0;
    } else if (nHeight >= 20 && nHeight <= 1000) {
...
{
    RenameThread("surfnode-scriptch");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	uiInterface.ThreadSafeMessageBox(_("You are on the wrong chain.  Please check debug.log for more information on resolving this.  Shutting down."), "", CClientUIInterface::MSG_ERROR);
        LogPrintf("Your block database is from a forked version of the correct chain.  Please visit https://github.com/SFNCoin/bootstrap to avoid any potential loss of future rewards and get on the proper chain.\n");
        StartShutdown();
        return false;
      }
    }
