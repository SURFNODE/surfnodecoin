...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SURFNODE users should be well used to small numbers with many decimal places by now :)
...
void ThreadSocketHandler()
{
    ...
    while (true) {
        ...
        BOOST_FOREACH (CNode* pnode, vNodesCopy) {
            ...
            if (nTime - pnode->nTimeConnected > 1) {
                if (pnode->nLastRecv == 0 || pnode->nLastSend == 0) {
                    ...
                    pnode->fDisconnect = true;
                } else if (nTime - pnode->nLastRecv > (pnode->nVersion > BIP0031_VERSION ? TIMEOUT_INTERVAL : 5 * 60)) {
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "SURFNODE " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SFN is probably already running."), addrBind.ToString());
        else
